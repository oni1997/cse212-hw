...
public static class Recursion
{
    ...
    public static void PermutationsChoose(List<string> results, string letters, int size, string word = "")
    {
        ...
        // Recursive case: try each letter from the remaining letters
        for (int i = 0; i < letters.Length; i++)
        {
            // Choose the current letter
            char currentLetter = letters[i];

            // Create a new string with the remaining letters (excluding current)
            string remainingLetters = letters.Remove(i, 1);

            // Recurse with the current letter added to the word and remaining letters
    ...
    public static void WildcardBinary(string pattern, List<string> results)
    {
        ...
        // Create pattern with '0' replacing the first wildcard
        string pattern0 = pattern[..wildcardIndex] + "0" + pattern[(wildcardIndex + 1)..];
        WildcardBinary(pattern0, results);

        // Create pattern with '1' replacing the first wildcard
    ...
    public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<ValueTuple<int, int>>? currPath = null)
    {
        ...
        if (maze.IsEnd(x, y))
        {
            ...
            // Remove current position from path (backtrack) before returning
